...
interface CreateSavedListModalProps {
  visible: boolean;
  onClose: () => void;
  onSave: (name: string, description: string | null, items: SavedListItemInput[]) => Promise<void>;
}

export const CreateSavedListModal: React.FC<CreateSavedListModalProps> = ({
  visible,
  onClose,
  onSave,
}) => {
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave(name.trim(), description.trim() || null, items);
      // Reset form
      setName('');
      setDescription('');
      setItems([]);
          ...
          {items.length > 0 && (
            <View style={[styles.section, { backgroundColor: theme.colors.card }]}>
              ...
              {items.map((item, index) => (
                <View key={index} style={[styles.itemCard, { backgroundColor: theme.colors.background }]}>
